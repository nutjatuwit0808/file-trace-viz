#!/usr/bin/env bash
# FileTraceViz — Cursor beforeReadFile hook.
# Writes a `read` entry; keeps generation_id verbatim as an extra field for debugging
# (plan 1.4). File path stays raw — the Phase 2 mapper owns normalization.
# Fail-open by design (CONVENTIONS.md §5): every path exits 0, stdout stays empty so
# Cursor falls back to its default "allow" behavior.
...
log_err() {
  ...
  printf '%s [cursor/log-file-read] %s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$1" \
...
file_path=$(printf '%s' "$payload" | jq -r '.file_path // empty' 2>/dev/null) || file_path=""
[ -n "$file_path" ] || { log_err "no file_path in payload"; exit 0; }

mkdir -p "$LOG_DIR" "$STATE_DIR" 2>/dev/null || { log_err "mkdir failed"; exit 0; }

# Reads only observe the counter; missing counter = session predates hooks = turn 1.
...
entry=$(printf '%s' "$payload" | jq -c --arg ts "$ts" --argjson turn "$turn" '{
  ...
  tool: "cursor", event: "read", file_path: .file_path,
  agent_id: null, generation_id: (.generation_id // null)
}' 2>/dev/null) || entry=""
